...
const StorePage = () => {
  ...
  const join = () => {
    dispatch(setLoader());
    if (!inQueue) {
      dispatch(joinQueue(location.pathname.split('/')[2])).then(() => {
        navigate('/');
      });
    } else {
      var today = new Date();
      var time = parseInt(today.getHours() * 60 + today.getMinutes());
  ...
  const handleSearchChange = (event) => {
    ...
    const filtered = allQueues.filter((store) =>
  ...
  return (
    <>
          ...
          <tr>
            <td className={styles.left}>
              <h1 className={homeStyles.mainHead} style={{ margin: '0' }}>
                {Details.name ? Details.name : 'Store Name'}
              </h1>
              <table>
                <tbody>
                  <tr>
                    ...
                    <td style={{ padding: '10px' }}>
                      <div>
                        ...
                        <span
                          ...
                          style={{ margin: '0', position: 'relative', bottom: '6px', padding: '2px 5px' }}
                        >
                          {arr[0] ? arr[0] : 0} min
                        </span>
                  ...
                  <tr>
                    ...
                    <td style={{ padding: '10px' }}>
                      <div>
                        ...
                        <span
                          className={homeStyles.yellowCapsule}
                          style={{ margin: '0', position: 'relative', bottom: '10px', padding: '2px 5px' }}
                        >
                          {bT} mins
                        </span>
              ...
              />{' '}
              Open {Details.from ? Details.from : 9}.00AM-{Details.to ? Details.to : 6}.00PM
              <div>
                <br />
                {inQueue ? (
                  <div>
                ...
                <p style={{ width: '90%' }}>
                  {Details.Address
                    ? Details.Address
                    : 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vestibulum efficitur feugiat ex sed gravida. Proin eu orci varius, dictum erat ac, ullamcorper arcu. Aliquam erat volutpat.Nam sagittis leo '}
                </p>
              </div>
            </td>
      ...
      <button
        className={inQueue ? homeStyles.leaveButton : homeStyles.enterButton}
        style={{ width: '20%', marginLeft: '40%', marginTop: '10px' }}
        onClick={() => join()}
      >
